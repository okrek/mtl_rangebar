...
class PinView extends View {
    ...
    public void init (Context ctx, float y, float pinRadiusDP, int pinColor, int textColor,
                      float circleRadius, int circleColor, float minFont, float maxFont, float pinPadding) {
        init(ctx, y, pinRadiusDP, pinColor, textColor, circleRadius, circleColor, minFont, maxFont);
    ...
    public void init(Context ctx, float y, float pinRadiusDP, int pinColor, int textColor,
        ...
        int textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                mMaxPinFont, mRes.getDisplayMetrics());
    ...
    public void draw(Canvas canvas) {
        ...
        if (mPinRadiusPx > 0) {
            String text = mValue;
            double textWidth = mTextPaint.measureText(mValue) + mPinPadding;
            double halfBoundsWidth = Math.max(mPinRadiusPx, textWidth / 2);
            double leftEdge = mX - halfBoundsWidth;
            double rightEdge = mX + halfBoundsWidth;
            ...
            if (rightEdge > canvas.getClipBounds().right) {
                leftEdge -= rightEdge - canvas.getClipBounds().right;
                diff = rightEdge - canvas.getClipBounds().right;
                rightEdge = canvas.getClipBounds().right;
            }

            mBounds.set((int) leftEdge,
                    (int) mY - (mPinRadiusPx * 2) - (int) mPinPadding,
                    (int) rightEdge, (int) mY - (int) mPinPadding);
            ...
            canvas.drawText(text,
                    (float) (mX - diff), mY - mPinRadiusPx - mPinPadding + mTextYPadding,
                    mTextPaint);
        }
        super.draw(canvas);
    }
