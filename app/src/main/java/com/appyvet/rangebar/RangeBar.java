...
public class RangeBar extends View {
    ...
    private PinTextFormatter mPinTextFormatter = new PinTextFormatter() {
        ...
        public String getText(String value) {
            double pinValue = Double.parseDouble(value);
            if (pinValue == Math.round(pinValue)) {
                return String.format("%d", Math.round(pinValue));
            }
            return String.format(Locale.ENGLISH, "%.2f", pinValue);
        }
    };
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mIsRangeBar) {
            ...
            mLeftThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                    mCircleColor, mMinPinFont, mMaxPinFont, mPinPadding);
        }
        ...
        if (mArePinsTemporary) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    Log.d("Animation fraction", String.valueOf(animation.getAnimatedFraction()));
                    invalidate();
                }
            });
            animator.start();
